## File to create an instance of the Flask object to set up extended functionality
...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sample_database'
auth = HTTPTokenAuth(scheme='Token')
db = SQLAlchemy(app)

from .models import Task, Profile

INC_DATE_FMT = '%Y-%m-%d %H:%M:%S'
...
def register():
    needed = ['username', 'email', 'pw1', 'pw2']
    if all([key in request.form for key in needed]):
        username = request.form['username']
        profile = get_profile(username)
        if not profile:
            if request.form['pw1'] == request.form['pw2']:
                new_profile = Profile(username=username, email=request.form['email'], password=hasher.hash(request.form['pw1']))
                db.session.add(new_profile)
                db.session.commit()
                response = Response(response=json.dumps({'msg': 'Profile created'}), status=201, mimetype='application/json')
        ...
        response = jsonift({'error': f'Username {username} is already taken'})
        response.status_code = 400
        return response
